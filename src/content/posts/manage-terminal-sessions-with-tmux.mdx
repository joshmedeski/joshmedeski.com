...
`tmux` is extremely powerful. We covered the basic commands for creating sessions, configuring options, setting up macOS shortcuts with Alacritty, and adding your first plugin. There is much more to cover so I recommend checking out my other content on tmux, like [Smart tmux sessions with zoxide and fzf ](http://localhost:8888/posts/smart-tmux-sessions-with-zoxide-and-fzf). You can also run `man tmux` in the terminal to open the tmux manual and dive deep into everything tmux has to offer.
